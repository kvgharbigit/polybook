...
export class LanguagePackService {
  ...
  /**
   * Ensure database has test data for development
   */
  private static async ensureDatabaseHasData(databasePath: string, pack: LanguagePackManifest): Promise<void> {
    console.log(`📦 ensureDatabaseHasData called for ${pack.id} at ${databasePath}`);
    try {
      const SQLite = await import('expo-sqlite');
      console.log(`📦 Opening database for data check...`);
      const db = await SQLite.openDatabaseAsync(databasePath);
      
      // Check if database already has data
      const tables = await db.getAllAsync("SELECT name FROM sqlite_master WHERE type='table'");
      console.log(`📦 Checking database for existing data: ${tables.length} tables found`);
      
      if (tables.length === 0) {
        console.log(`📦 Database is empty, creating test data for ${pack.id}...`);
        
        // Create the dict table with real Spanish-English entries
        console.log(`📦 Creating real dictionary data for ${pack.id}...`);
        
        // Note: In production, this should be replaced with proper build system
        // For now, we'll create some sample real entries
        await db.execAsync(`
          CREATE TABLE dict (
          ...
          INSERT INTO dict (lemma, def) VALUES 
          ('hello', 'hola - a greeting used when meeting someone'),
          ('house', 'casa - a building for human habitation'),
          ('water', 'agua - colorless, transparent liquid'),
          ('book', 'libro - written or printed work'),
          ('good', 'bueno - having the right or desired qualities'),
          ('time', 'tiempo - indefinite continued progress'),
          ('person', 'persona - individual human being'),
          ('work', 'trabajo - activity involving effort'),
          ('day', 'día - period of 24 hours'),
          ('life', 'vida - condition of living things'),
          ('love', 'amor - deep affection or fondness'),
          ('friend', 'amigo - a person you know well and like'),
          ('family', 'familia - group of related people'),
          ('money', 'dinero - medium of exchange'),
          ('food', 'comida - something you eat'),
          ('school', 'escuela - place of learning'),
          ('car', 'coche - motor vehicle'),
          ('beautiful', 'hermoso - pleasing to look at'),
          ('happy', 'feliz - feeling joy'),
        ...
        const count = await db.getAllAsync('SELECT COUNT(*) as count FROM dict');
        console.log(`📦 Created test database with ${count[0].count} entries`);
        
      } else {
        const hasDict = tables.some(t => t.name === 'dict');
        if (hasDict) {
          const count = await db.getAllAsync('SELECT COUNT(*) as count FROM dict');
          console.log(`📦 Database already has ${count[0].count} entries in dict table`);
        } else {
          console.log(`📦 Database has tables but no dict table: ${tables.map(t => t.name).join(', ')}`);
        }
      }
      
    } catch (error) {
      console.error(`📦 Error checking/creating test data:`, error);
      // Don't throw - this is optional development data
    }
  }
}